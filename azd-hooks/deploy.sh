...
if [[ $SERVICE_NAME =~ ^[a-z0-9-]{3,20}$ ]]; then
    ...
    echo "service name $SERVICE_NAME is invalid - only numbers, lower case and hyphens, min 3 and max 20 characters allowed - aborting"
    exit 0;
fi
...
# Map service name to source directory (handle hyphens -> underscores)
SRC_DIR_NAME=$(echo "$SERVICE_NAME" | tr '-' '_')
...
az acr build --subscription ${AZURE_SUBSCRIPTION_ID} --registry ${AZURE_CONTAINER_REGISTRY_NAME} --image $SERVICE_NAME:latest ./src/$SRC_DIR_NAME
...
URI=$(az deployment group create -g $RESOURCE_GROUP -f ./infra/app/web.bicep \
          ...
          -p openaiName=$OPENAI_NAME -p searchName=$SEARCH_NAME -p storageAccountName=$STORAGE_NAME -p identityName=$IDENTITY_NAME -p imageName=$IMAGE_NAME --query properties.outputs.uri.value)
